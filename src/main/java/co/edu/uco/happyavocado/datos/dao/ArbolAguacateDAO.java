package co.edu.uco.happyavocado.datos.dao;

import co.edu.uco.happyavocado.datos.*;
import co.edu.uco.happyavocado.entidad.AbonoArbolAguacateEntidad;
import co.edu.uco.happyavocado.entidad.ArbolAguacateEntidad;

import java.util.UUID;

public interface ArbolAguacateDAO extends
        ConsultarPorIdDAO<ArbolAguacateEntidad, UUID>,
        ConsultarTodosDAO<ArbolAguacateEntidad>,
        ConsultarPorFiltroDAO<ArbolAguacateEntidad>,
        CrearDAO<ArbolAguacateEntidad>,
        ActualizarDAO<ArbolAguacateEntidad> {
}